 ...
 */
class delete_minicomponent_registry
{
    ...
    public function regenerate_registry($force_reload_allowed = false)
    {
        ...
        if ($task == 'deleteregistry' || $task == 'save_site_settings') {
     ...
     $dir = "*";
     array_map('unlink', glob(JOMRES_TEMP_ABSPATH .JRDS . $dir ));
     rmdir($dir);
     
      $weather_path = JOMRES_TEMP_ABSPATH.JRDS.'weather_cache'.JRDS;
			
			if (is_dir($weather_path)) {
                emptyDir($weather_path);
            }
       
			$qr_codes_path = JOMRES_TEMP_ABSPATH.JRDS.'qr_codes'.JRDS;
			
			if (is_dir($qr_codes_path)) {
                emptyDir($qr_codes_path);
            }
            
      $geolocation_path = JOMRES_TEMP_ABSPATH.JRDS.'geolocation_path'.JRDS;
			
			if (is_dir($geolocation_path)) {
                emptyDir($geolocation_path);             
            }	
